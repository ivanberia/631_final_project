...
\section{Assessment}
We verified our implementation on a couple different small test programs.\\
...
In our assessments, we verified that various assertions resulted how we expected.  We added assertions that we expected to be either true or false and checked that the output was as expected.  When an assertion was valid, the program printed sample inputs for the assertion.  When the assertion was not valid, the program would instead throw an exception to halt execution of the program and notify the user that an assertion had failed.
